      ...
      <div id="contenido">
        ...
        <div id="section">
          <div class="article1"><br>
            <h3>Habilitar Profesor</h3>    
            <form style="border-radius:1em; margin-left: 200px; margin-right: 200px; margin-top: 30px; text-align: center;" action="Includes/HabP.php" method="POST">
              <fieldset>
                <label >Habilitar Profesor</label><br>
                  <select name="no_empleado" id="no_empleado">
                    <option value="0">Seleccione:</option>
                    <?php  
                    require 'Includes/conexion.php';                  
                    $query1 = $conn -> query ("SELECT * FROM datos_prof WHERE status=0;");
                    while ($valores1 = mysqli_fetch_array($query1)) {                      
                      echo '<option value="'.$valores1['no_empleado'].'">'.$valores1['no_empleado'].'</option>';
                    }
                    ?>                                
                  </select><br><br>
			...
			<form style="border-radius: 10em; margin-left: 200px; margin-right: 200px; margin-top: 30px; text-align: center;" action="Includes/DesHP.php" method="POST">
              <fieldset>                
                <label>Desabilitar Profesor</label><br>
                  <select name="no_empleado" id="no_empleado">
                    <option value="0">Seleccione:</option>
                    <?php  
                    require 'Includes/conexion.php';                  
                    $query2 = $conn -> query ("SELECT * FROM datos_prof WHERE status=1;");
                    while ($valores2 = mysqli_fetch_array($query2)) {                      
                      echo '<option value="'.$valores2['no_empleado'].'">'.$valores2['no_empleado'].'</option>';
                    }
                    ?>                                
                  </select><br><br>
